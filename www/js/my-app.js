...
function login(e, page) 
{
    $$('#btn-login').on('click', function() {
        ...
        if (email == '' || pass == '') 
        {
            ...
            $$("#modnotif_login_pesan").html(pesan + " tidak boleh kosong !");
            BukaModal("modnotif_login");
            return;
        }
        else 
        {
...
function onBackKeyDown() {
	if(app.views.main.history.length == 1 || app.views.main.router.url == '/login/') {
...
function MasukKeranjang(content)
{
    ...
    else
    {
        ...
        cekKeranjang();
        ////
        if(content == "KategoriDetil_MenuModal")
        {
            KategoriDetil(localStorage.kategori_id,localStorage.kategori_nama);
        }
    }
}
...
{    
    ...
    if (halaman == "cabang") //DONE
    {
            ...
            (storephone == "" || storephone == null || isNaN(storephone)) )
        {
            ...
            $$("#modnotif_"+halaman+"_pesan").html(pesan + " tidak boleh kosong !");
            BukaModal("modnotif_cabang");
            var btn = document.getElementById(halaman + 'detil_tutupnotif');
    ...
    else if (halaman == "kategori") //DONE
    {
        ...
        if (nama == "" || nama == null)
        {
            ...
            $$("#modnotif_"+ halaman +"_pesan").html(pesan);
    ...
    app.request({
        ...
        success: function(data) {
            ...
            if (obj.status == true)
            {
                ...
                $$("#modnotif_"+ halaman +"_pesan").html('Berhasil menambahkan/mengubah '+ nama +' baru!');
                var btn = document.getElementById(halaman + 'detil_tutupnotif');

                if (halaman == 'kategori')
                {
                    btn.setAttribute('href', '/'+halaman+'/kategoriedit');
                }
                else if (halaman == "jenispembayaran")
                {
                    btn.setAttribute('href', '/pembayaran/');
                }
                else
                {
                    btn.setAttribute('href', '/'+halaman+'/');
                }
            ...
            {
                $$("#modnotif_"+ halaman +"_pesan").html('Gagal menambahkan/mengubah '+ nama +' !');
                var btn = document.getElementById(halaman+'detil_tutupnotif');
                btn.removeAttribute('href');
                BukaModal("modnotif_" + halaman);
            }
...
{    
    ...
    else if (halaman == "karyawan")
    {
        ...
        if ((nama == "" || nama == null) || 
            ...
            (karyawan_password == "" || karyawan_password == null) )
        {
            $$("#modnotif_"+ halaman +"_pesan").html('Kolom tidak boleh kosong!');
            BukaModal("modnotif_" + halaman);
            var btn = document.getElementById('karyawandetil_tutupnotif');
            btn.removeAttribute('href');
            return;
        }
    }
...
function HapusData(halaman)
{
    ...
    app.request({
        ...
        success: function(data) {
            ...
            if (obj['status'] == true)
            {                                               
                TutupModal('mod_' + halaman + '_hapus');      

                $$("#modnotif_"+ halaman +"_pesan").html('Berhasil menghapus ' +halaman+' !');
                var btn = document.getElementById(halaman + 'detil_tutupnotif');
                btn.setAttribute('href', '/'+halaman+'/');
            ...
            else
            {
                ...
                $$("#modnotif_"+ halaman +"_pesan").html('Gagal menghapus ' + halaman + ' !');
                var btn = document.getElementById(halaman + 'detil_tutupnotif');
                btn.removeAttribute('href');
    ...
    app.request({
        ...
        success: function(data) {
            ...
            if (obj['status'] == true)
            {
                for(var i =0; i < obj['data'].length ; i++)
                {   
                    var image = obj['data'][i]['menu_id'] + "_" + obj['data'][i]['menu_name'] + "." + obj['data'][i]['menu_img_ext'];
...
{    
    app.request({
        ...
        success: function(data) {
            ...
            document.getElementById("karyawandetil_password").value = obj['data'][0]['password'];
            document.getElementById("karyawan_jabatan_combobox").value = obj['data'][0]['user_role'];
            document.getElementById("karyawan_cabang_combobox").value = obj['data'][0]['store_id'];
    ...
    app.request({
        ...
        success: function(data) {
            ...
            if(obj['result'] == "Berhasil")
            {
                ...
                for(var i =0; i < obj['data'].length; i++)
                {
                    var image = obj['data'][i]['payment_type_id'] + "_" + obj['data'][i]['payment_type_name'] + "." + obj['data'][i]['payment_img_ext'];
                    var image_url = "http://192.168.0.15/myorder/images/" + image;
    ...
    app.request({
        ...
        success: function(data) {
            ...
            if (localStorage.user_role == "Pelanggan")
            {
                ...
                $$("#keranjang_biaya_kirim").html("Rp " + formatRupiah(biayaAntar));
            }
...
function pesan() {
    ...
    if (payment == 0)
    {
        app.dialog.alert('Pilih jenis pembayaran dahulu.');
    }
    else
    {
        if (Array_idMenu == "")
        {
            app.dialog.alert('Keranjang kosong, tambahkan menu!');
        }
        else
        {
            app.request({
        ...
        success: function(data) {                        
            ...
            if (halaman == "beranda")
            {
                if (obj['status'] == true)
                {
                    ...
                    for(var i =0; i < obj['data'].length; i++)
                    {                 
                        var image = obj['data'][i]['banner_id'] + "_" + obj['data'][i]['banner_name'] + "." + obj['data'][i]['banner_img_ext'];                        
                        var image_url = "http://192.168.0.15/myorder/images/" + image;
                        ...
                        if(obj['data'][i]['category_aim_id'] == null || obj['data'][i]['category_aim_id'] == "")
                        {
                            $$("#beranda_banner_promo").append(                        
                                '<div>'+
                                    '<img class="banner" src="' + image_url + '"' +" onerror='this.src=" + '"./img/drawable-ldpi/1200x628.png"' + "'" +' alt="" onclick="ShowModalMenu('+ obj['data'][i]['menu_aim_id'] + ",'Beranda_MenuModal')" + '">'+
                                '</div>'
                            )
                        }
                        else
                            ...
                            $$("#beranda_banner_promo").append(                        
                                '<div>'+
                                    '<a style="width:100%" href="/kategoridetil/'+ obj['data'][i]['category_aim_id'] +'/'+ "'test'" +'">' +
                                        '<img class="banner" src="' + image_url + '"' +" onerror='this.src=" + '"./img/drawable-ldpi/1200x628.png"' + "'" +' alt="">' +                                        
                                    '</a>' +
                                '</div>'
                            )
                        }
                    ...
                    for(var i =0; i < obj['data'].length; i++)
                    {
                        ...
                        $$(".banner-list").append(                        
                            '<div class="card" style=" margin-bottom: 1rem;">' +
                                '<img style="width: 100%;" src="' + image_url + '"' +" onerror='this.src=" + '"./img/drawable-ldpi/1200x628.png"' + "'" +' alt="">' +
                                '<div class="card-content">' +
                                    '<div class="row">' +
...
function camera(halaman)
{
    ...
    function onSuccess(imageData) {      
        ...
        plugins.crop.promise(imageData, { quality: 50, targetWidth: 300, targetHeight: 300, correctOrientation: true })
        .then(function success (newPath) {
            $$("#"+ halaman +"_propic").attr('src', newPath );
            $$("#"+ halaman +"_propic").css({'width':'100px', 'height':'100px'});
        })
        .catch(function fail (err) {
            app.dialog.alert('Failed because: ' + err);
        })
    }
    ...
    app.request({
        ...
        success: function(data) {              
            ...
            else if (halaman == "cabang")
            {
                ...
                {
                    $$('#admin_cabanglist').html("");
                    $$('#admin_cabanglist').html('<h4 id="statusadminCabangKosong">Tidak ada hasil ditemukan!</h4>');
                    $$('#statusadminCabangKosong').css({'text-align':'center','font-weight':'700', 'margin':'auto', 'color':'gray', 'margin-top':'13rem'});
                }
